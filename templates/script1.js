...
async function loadModel() {
  ...
  // Now you can use the model for inference or any other tasks.
}
...
function removeElements(landmarks, elements) {
  ...
  function onResults(results) {
    ...
    drawLandmarks(
      canvasCtx, results.rightHandLandmarks, {
        ...
        lineWidth: 2,
        radius: (landmark) => {
          return lerp(landmark.z, -0.15, 0.1, 10, 1);
        },
      }
    );
    ...
    // Add the pose data to the array
    poseData.push(pose);
  ...
  function startWebcamAndDetection() {
    ...
    new ControlPanel(controlsElement, {
      ...
      new StaticText({ title: 'ASL Recognization' }),
      fpsControl,
