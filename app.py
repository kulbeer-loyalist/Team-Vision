...
rdsConnection = create_engine("mysql+mysqldb://admin:passwordRDS@visionrds.ch0vw6wkyqa3.ca-central-1.rds.amazonaws.com/")
...
app.config['SQLALCHEMY_DATABASE_URI'] = f'mysql://admin:passwordRDS@visionrds.ch0vw6wkyqa3.ca-central-1.rds.amazonaws.com/vision'
...
def login():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first()  # checking if the user email is in the database
        if user:
            if check_password_hash(user.password, password):  # checking in the database if the user password is correct
                ...
                return redirect(url_for('user', user=current_user))  # redirecting to user page if the login is successful
            else:
